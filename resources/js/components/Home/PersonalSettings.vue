    ...
    <div class="d-flex flex-column" style="width: max-content">
        ...
        <form @submit.prevent="send" method="post">
            ...
            <div class="form-group row m-0 p-0">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="organization" v-model="organization" placeholder="Название организации">
                </div>
            </div>
            ...
            <div class="form-group row m-0 p-0">
                ...
                <div class="col-sm-10">
                    ...
                    <div v-if="city.length !== 0 && close_dropdown === 0" class="city_dropdown text-left w-50 overflow-auto">
            ...
            <div class="form-group row m-0 p-0">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="adress" v-model="adress" placeholder="Адрес">
                </div>
            </div>
            ...
            <div class="form-group row m-0 p-0">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="phone" v-model="phone" placeholder="Телефон">
                </div>
            </div>
            ...
            <div class="form-group row m-0 p-0">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="site" v-model="site" placeholder="Веб-сайт(если есть)">
                </div>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                city:'',//значение, привязанное к инпуту city
                placeholder_city:'Населенный пункт',//отображается в качестве плейсхолдера инпута city
                close_dropdown:0,
                adress:'',
                phone:'',
                site:'',
                ...
                initial_coords: [// запоминаю начальное значение координат, чтобы перед отправкой формы, удостовериться, что они введены
                    65.841076,
                    98.133271
                ],
        ...
        methods: {
            ...
            send(){
                let options={
                    ...
                    'alias':this.translit(this.organization),
                    'city':this.city,
                    'adress':this.adress,
                    'phone':this.phone,
                    'site':this.site,
                    'products':this.moto_brands
                }
                //Валидация
                let validator = this.validator
                this.errors=[]
                validator(String(this.coords),String(this.initial_coords),'Пожалуйста поставьте маркер на карту')
                validator(this.organization,'','Пожалуйста укажите организацию')
                validator(this.city,'','Пожалуйста укажите ваш город')
                validator(this.adress,'','Пожалуйста укажите адрес')
                validator(this.phone,'','Пожалуйста укажите ваш телефон')
                validator(this.moto_brands.length,0,'Пожалуйста укажите тип техники')
                let counter = 0
                for (let city of this.cities){
                   if (city.title.toLowerCase()===this.city.toLowerCase()){counter++}
                }
                if(counter === 0){this.errors.push('Выберете город из выпадающего меню')}
                ...
                //Запрос
                axios
                    .post('/home/personal/new_dealer',options)
                    .then(response => {
